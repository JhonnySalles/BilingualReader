...
class LibraryBookPopupOrderTest {
    ...
    @Test
    fun testClickChangesStateFromUncheckedToChecked() {
        val scenario = launchFragmentInContainer<LibraryBookPopupOrder>(themeResId = R.style.AppBackground)
    ...
    fun testDoubleClickChangesStateToIndeterminate() {
        ...
        onView(withId(R.id.popup_library_order_book_date)).perform(click()) // Unchecked -> Checked
        onView(withId(R.id.popup_library_order_book_date)).perform(click()) // Checked -> Indeterminate
        
        onView(withId(R.id.popup_library_order_book_date)).check(matches(withTriStateState(TriStateCheckBox.STATE_INDETERMINATE)))
    }
}
