...
class DocContext : PdfContext() {
    private val LOGGER = LoggerFactory.getLogger(DocContext::class.java)
    ...
    override fun openDocumentInner(fileName: String, password: String): CodecDocument? {
        ...
        try {
            val contentResult = runBlocking(br.com.ebook.core.EbookDispatcher.dispatcher) {
                DocBookExtractor.extractContent(fileName, CacheZipUtils.CACHE_BOOK_DIR?.path ?: "")
            }
            ...
            LOGGER.info("doc new file name: {}", finalFileName)
        } catch (e: Exception) {
            LOGGER.error("Error open document inner in DocContext: {}", e.message, e)
        }
