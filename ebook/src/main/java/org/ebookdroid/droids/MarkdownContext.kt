...
class MarkdownContext : PdfContext() {
    private val LOGGER = LoggerFactory.getLogger(MarkdownContext::class.java)
    ...
    override fun openDocumentInner(fileName: String, password: String): CodecDocument? {
        ...
        try {
            val contentResult = runBlocking(br.com.ebook.core.EbookDispatcher.dispatcher) {
                MarkdownBookExtractor.extractContent(fileName, CacheZipUtils.CACHE_BOOK_DIR?.path ?: "")
            }
            ...
            LOGGER.info("markdown new file name: {}", finalFileName)
        } catch (e: Exception) {
            LOGGER.error("Error open document inner in MarkdownContext: {}", e.message, e)
        }
