...
class OdtContext : PdfContext() {
    private val LOGGER = LoggerFactory.getLogger(OdtContext::class.java)
    ...
    override fun openDocumentInner(fileName: String, password: String): CodecDocument? {
        ...
        try {
            val contentResult = runBlocking(br.com.ebook.core.EbookDispatcher.dispatcher) {
                OdtBookExtractor.extractContent(fileName, CacheZipUtils.CACHE_BOOK_DIR?.path ?: "")
            }
            ...
            LOGGER.info("odt new file name: {}", finalFileName)
        } catch (e: Exception) {
            LOGGER.error("Error open document inner in OdtContext: {}", e.message, e)
        }
